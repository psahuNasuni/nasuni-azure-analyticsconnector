...
VOLUME_NAME="$2"
echo "curl -X GET 'https://${NAC_DISCOVERY_FUNCTION_APP}/api/IndexFunction?nmc_volume_name=${VOLUME_NAME}' -H 'Content-Type:application/json'"
n=0
until [ "$n" -ge 5 ]
do
    COMMAND=$(curl -X GET "https://${NAC_DISCOVERY_FUNCTION_APP}/api/IndexFunction?nmc_volume_name=${VOLUME_NAME}" -H "Content-Type:application/json")
    ### COMMAND=$(curl -X GET "https://nasuni-function-app-9ed7bb.azurewebsites.net/api/IndexFunction?nmc_volume_name=${VOLUME_NAME}" -H "Content-Type:application/json")
